...
import { conversionFunnelData, weightedForecastData, fmtTr } from "./reportsData";
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-5 border-t border-[#E8E7E2] pt-4 text-center">
              {conversionFunnelData.map((d) => (
                <div key={d.stage} className="flex flex-col gap-1 border-r border-[#E8E7E2] last:border-0">
                  <span className="text-[#6B6B67]" style={{ fontSize: 10 }}>{d.stage.split(". ")[1]}</span>
        ...
        <div className="flex flex-col gap-4 justify-between">
          <div className="bg-white rounded-[10px] border border-[#E8E7E2] p-5 flex flex-col gap-3 flex-1">
            <h4 className="text-[#1A1A18] font-bold" style={{ fontSize: 13 }}>Phân tích điểm nghẽn (Bottlenecks)</h4>
            <div className="space-y-3 mt-1.5">
              <div className="flex items-start gap-2.5">
                <span className="size-5 rounded-full bg-[#FEE2E2] text-[#A32D2D] font-bold flex items-center justify-center text-xs shrink-0">!</span>
                <div>
                  <p className="text-[#1A1A18] font-medium" style={{ fontSize: 12 }}>
                    {"Tỷ lệ Qualified -> Proposal giảm mạnh (-44%)"}
                  </p>
                  <p className="text-[#6B6B67]" style={{ fontSize: 11 }}>
                    Nhiều khách hàng tiềm năng được định danh nhưng không nhận được/chưa đồng ý đề xuất giải pháp.
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <span className="size-5 rounded-full bg-[#E6F6F0] text-[#1D9E75] font-bold flex items-center justify-center text-xs shrink-0">✓</span>
                <div>
                  <p className="text-[#1A1A18] font-medium" style={{ fontSize: 12 }}>
                    {"Tỷ lệ Proposal -> Won cải thiện rõ rệt (+5%)"}
                  </p>
                  <p className="text-[#6B6B67]" style={{ fontSize: 11 }}>
                    Một khi khách hàng đã đồng ý nhận báo giá, tỷ lệ chốt deal thành công là rất cao (đạt 56%).
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-[#534AB7] rounded-[10px] p-5 text-white flex flex-col justify-between" style={{ minHeight: 120 }}>
            <div>
              <p className="opacity-80" style={{ fontSize: 11 }}>Xác suất chốt deal trung bình (Pipeline Win Velocity)</p>
              <p className="font-extrabold mt-1" style={{ fontSize: 26 }}>15.0%</p>
            </div>
            <p className="opacity-70 border-t border-white/20 pt-2.5 mt-2" style={{ fontSize: 10 }}>
              Cứ 100 Lead đầu phễu sẽ tạo ra 15 deal thắng. Cần thêm 80 Lead mới để đạt doanh thu target tháng tới.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={weightedForecastData} margin={{ top: 10, right: 16, left: 0, bottom: 5 }}>
