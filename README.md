...
Hệ thống quản lý quan hệ khách hàng (CRM) dưới dạng phần mềm dịch vụ (SaaS) đa doanh nghiệp (Multi-tenant), được thiết kế tối ưu hóa quy trình bán hàng qua Sales Pipeline (Kanban Board), quản lý công việc và tích hợp sâu trí tuệ nhân tạo (Generative AI) nhằm tự động hóa các tác vụ chăm sóc khách hàng hàng ngày cho doanh nghiệp vừa và nhỏ (SMEs).

🚀 **Link Live Demo:** [https://your-crm-saas-demo.link](https://your-crm-saas-demo.link) *(Đang cập nhật)*  
👥 **Tài khoản chạy thử nghiệm:**
*   **Tenant 1 (Công ty Công nghệ):** `admin@tenant1.com` / Mật khẩu: `Password123`
*   **Tenant 2 (Công ty Bán lẻ):** `admin@tenant2.com` / Mật khẩu: `Password123`
...
| --- | --- |
| ![Dashboard](https://placehold.co/600x380/2b303b/ffffff/png?text=Dashboard+Analytics+Screen) | ![Kanban Board](https://placehold.co/600x380/2b303b/ffffff/png?text=Kanban+Deals+Pipeline) |
...
| --- | --- |
| ![AI Integration](https://placehold.co/600x380/2b303b/ffffff/png?text=AI+Meeting+Brief+Summary) | ![Tenant Settings](https://placehold.co/600x380/2b303b/ffffff/png?text=Multi-tenant+Isolation+Settings) |
...
graph TD
    ...
    Worker -->|6. Run AI Analysis| OpenAI[OpenAI API GPT-4o-mini]
...
*   **AI Integration:** OpenAI API SDK (`gpt-4o-mini`) để phân tích ngôn ngữ tự nhiên và trích xuất thông tin.
...
*   **Thách thức:** Việc phân tích biên bản cuộc họp bằng OpenAI API có thể tốn từ 5 - 15 giây. Nếu gọi trực tiếp đồng bộ, Server sẽ bị treo luồng xử lý (blocking) và trình duyệt có thể bị quá thời gian chờ (timeout).
...
2. Tạo file cấu hình môi trường `.env`:
   ...
   cp .env.example .env
   ```
   *Cập nhật các biến số: `DATABASE_URL` kết nối PostgreSQL, `REDIS_HOST`/`REDIS_PORT` kết nối Redis và `OPENAI_API_KEY` của bạn.*
